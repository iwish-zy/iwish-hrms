...
async function GET() {
    ...
    const { data, error } = await supabase.from("onboarding_tasks").select("*").order("created_at", {
        ascending: false
    });
...
async function POST(request) {
    ...
    const { data, error } = await supabase.from("onboarding_tasks").insert(body).select().single();
    if (error) return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
        error: error.message
    }, {
        status: 500
